...
int main() {
    ...
    // 用 builder 创建 getelementptr 指令，参数解释：第一个参数传入 getelementptr 的指针参数，第二个参数传入取偏移的数组。
    // 在此条语句中，u0pLoad 为 i32*类型的指针，且只取一次偏移，偏移值为 0，因此返回的即为 u[0] 的地址
    ...
    // 参数解释：第一个参数传入 getelementptr 的指针参数，第二个参数传入取偏移的数组。
    // 在此条语句中，x 为 [1 x i32]*类型的指针，取两次偏移，两次偏移值都为 0，因此返回的即为 x[0] 的地址。
