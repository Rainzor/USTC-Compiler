...
void translate_main(CodeGen *codegen) {
    ...
    codegen->append_inst("st.b",{"$t0", "$fp", std::to_string(offset_map["%op0"])});
    codegen->append_inst("b fccd_exit");
    ...
    codegen->append_inst("fccd_exit", ASMInstruction::Label);
    /* %op1 = zext i1 %op0 to i32 */
    codegen->append_inst("%op1 = zext i1 %op0 to i32", ASMInstruction::Comment);
    // 将 %op0 的值从 i1 类型转换为 i32 类型, 并将结果写入到 %op1 对应的内存空间中
    offset_map["%op1"] = -24; // TODO: 请填空
    // TODO: 获得 %op0 的值, 然后进行转换, 最后将结果写入 %op1
    // 思考: 怎么转换? 需不需要显式地使用某些指令转换?
    codegen->append_inst("ld.b", {"$t0", "$fp", std::to_string(offset_map["%op0"])});
    codegen->append_inst("addi.w", {"$t1", "$t0", "0"});
    ...
    codegen->append_inst("st.b", {"$t1","$fp", std::to_string(offset_map["%op2"])});
    ...
    codegen->append_inst("ld.b", {"$t0", "$fp", std::to_string(offset_map["%op2"])});
    codegen->append_inst("beqz", {"$t0", ".main_label4"});
