...
void translate_callee(CodeGen *codegen) {
    ...
    // 保存返回地址
    codegen->append_inst("st.d $ra, $sp, -8");//返回地址存入栈中
    // 保存老的 fp
    codegen->append_inst("st.d $fp, $sp, -16");//老的基地址(栈底)fp存入新栈中，方便后续恢复
    // 设置新的 fp
    codegen->append_inst("addi.d $fp, $sp, 0");//main函数的栈顶作为新函数callee的栈底
    // 为栈帧分配空间. 思考: 为什么是 48 字节?
    codegen->append_inst("addi.d $sp, $sp, -48");//为栈帧分配空间，改变栈顶指针
...
void translate_main(CodeGen *codegen) {
    ...
    // TODO: 调用 callee 函数
    codegen->append_inst("bl callee");//这里将PC+4存入ra,即下一条指令的地址
    // 将返回值写入 %op0 对应的内存空间中
    codegen->append_inst("st.w",
