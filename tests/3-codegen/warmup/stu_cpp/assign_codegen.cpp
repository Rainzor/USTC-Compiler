...
void translate_main(CodeGen *codegen) {
    ...
    codegen->append_inst("st.d", {"$t0", "$fp", std::to_string(offset_map["%op1"])});
    ...
    // TODO: 先获得 %op1 的值, 然后获得 %op1 指向的空间的值, 最后将这个值写入%op3 对应的内存空间中
    codegen->append_inst("ld.d", {"$t0", "$fp", std::to_string(offset_map["%op1"])});
    codegen->append_inst("ld.w", {"$t1", "$t0", "0"});
    codegen->append_inst("st.w", {"$t1", "$fp", std::to_string(offset_map["%op3"])});

    /* %op4 = mul i32 %op3, 3 */
    codegen->append_inst("%op4 = mul i32 %op3, 3", ASMInstruction::Comment);
    offset_map["%op4"] = -88; // TODO: 请填空
    // TODO: 先获得 %op3 的值, 然后将其乘以 3, 最后将结果写入 %op4 对应的内存空间中
    codegen->append_inst("ld.w", {"$t0", "$fp", std::to_string(offset_map["%op3"])});
    codegen->append_inst("addi.w",{"$t1","$zero","3"});
    codegen->append_inst("mul.w", {"$t2", "$t1", "$t0"});
    codegen->append_inst("st.w", {"$t2", "$fp", std::to_string(offset_map["%op4"])});
    ...
    // TODO: 先获得 %op2 的值, 然后获得 %op2 指向的空间的值, 最后将这个值写入%op5 对应的内存空间中 
